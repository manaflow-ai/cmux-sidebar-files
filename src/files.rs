...
mod tests {
    ...
    fn lists_directories_first_and_sorts_each_kind() {
        ...
        create_dir(temp.path().join("z-dir")).unwrap();
        create_dir(temp.path().join("A-dir")).unwrap();
        write(temp.path().join("z-file"), "").unwrap();
        write(temp.path().join("B-file"), "").unwrap();
    ...
    fn hides_dotfiles_until_enabled() {
        let temp = tempdir().unwrap();
        create_dir(temp.path().join(".config")).unwrap();
        write(temp.path().join(".env"), "").unwrap();
        write(temp.path().join("visible"), "").unwrap();

        let hidden = list_directory(temp.path(), false).unwrap();
        assert_eq!(hidden.len(), 1);
        assert_eq!(hidden[0].name, "visible");

        let shown = list_directory(temp.path(), true).unwrap();
